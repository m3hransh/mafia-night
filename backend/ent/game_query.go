...
func (_q *GameQuery) QueryGameRoles() *GameRoleQuery {
...
func (_q *GameQuery) QueryVotes() *VoteQuery {
...
func (_q *GameQuery) WithGameRoles(opts ...func(*GameRoleQuery)) *GameQuery {
	query := (&GameRoleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GameQuery) WithVotes(opts ...func(*VoteQuery)) *GameQuery {
	query := (&VoteClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GameQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Game, error) {
	...
	if query := _q.withVotes; query != nil {
		if err := _q.loadVotes(ctx, query, nodes,
			func(n *Game) { n.Edges.Votes = []*Vote{} },
			func(n *Game, e *Vote) { n.Edges.Votes = append(n.Edges.Votes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *GameQuery) loadPlayers(ctx context.Context, query *PlayerQuery, nodes []*Game, init func(*Game), assign func(*Game, *Player)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(player.FieldGameID)
	}
...
func (_q *GameQuery) loadGameRoles(ctx context.Context, query *GameRoleQuery, nodes []*Game, init func(*Game), assign func(*Game, *GameRole)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Game)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(gamerole.FieldGameID)
	}
...
func (_q *GameQuery) loadVotes(ctx context.Context, query *VoteQuery, nodes []*Game, init func(*Game), assign func(*Game, *Vote)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Game)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(vote.FieldGameID)
	}
	query.Where(predicate.Vote(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(game.VotesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err

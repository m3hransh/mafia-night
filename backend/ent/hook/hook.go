...
type GameRoleFunc func(context.Context, *ent.GameRoleMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f GameRoleFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.GameRoleMutation); ok {
		return f(ctx, mv)
	}
